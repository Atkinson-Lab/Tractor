...
def extract_tracts(msp, vcf_prefix, zipped, num_ancs):
    ...
    for i in range(num_ancs):
        output_files[f"out{i}"] = f'{vcf_prefix}.anc{i}.vcf' #output extracted VCF for each ancestry into separate files
        output_files[f"outdos{i}"] = f'{vcf_prefix}.anc{i}.dosage.txt' # output dosages for each ancestry into separate files
        output_files[f"outancdos{i}"] = f'{vcf_prefix}.anc{i}.hapcount.txt'# output number of haplotype for each ancestry into separate files
...
if __name__ == "__main__":
    ...
    parser.add_argument('--vcf-prefix', help='path stem to RFmix input VCF with phased genotypes, not including .vcf suffix', required=True)
    parser.add_argument('--zipped', help='Input VCF is gzipped', action='store_true')
    parser.add_argument('--num-ancs', help='Number of continental ancestries in admixed populations', default=2)
